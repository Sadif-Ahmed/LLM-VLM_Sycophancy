#!/usr/bin/env bash
# Runs vqa_sycophancy_probe_grounded.py across all 3 prompt-set variants.
# One-third of run_vqa_variants.sh's 9 runs — meant to be launched standalone,
# in parallel with run_vqa_probe.sh / run_vqa_without_pres.sh, instead of
...
for ps in "${PROMPT_SETS[@]}"; do
  run "$SCRIPT_DIR/vqa_sycophancy_probe_grounded.py" \
