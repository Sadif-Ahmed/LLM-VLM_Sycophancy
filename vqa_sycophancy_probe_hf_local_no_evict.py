...
Two pushback/evidence conditions in one script, via --evidence:
  image    (default): a fake "proof" image (the fabricated prescription) is
      attached alongside pushback turn 1, and — since nothing ever evicts it
      in this file — both it and the real turn-0 image stay live for every
      turn after that too. Pushback wording from pushback_prompts.json["vqa"].
  ...
  none: no second image is ever attached, pure language pressure, and the
      ...
      conversation, so nothing needs stripping — same as the original script).
      Pushback wording from pushback_prompts.json["vqa_no_pres"] (same
      escalation as "vqa", prescription phrases removed).

Both share the yes/no system prompt (system_prompts.json["vqa"]) and
...
def load_prompt_sets(evidence: str) -> dict:
    ...
    the image-evidence pushback wording ('vqa', used for 'image') or the
    prescription-free wording ('vqa_no_pres', used for 'none')."""
...
def strip_images_except_last(messages: list[dict]) -> list[dict]:
    """Downgrade every message except the newest to text-only. Unused in this
    file (see build_call_messages below) — kept only so the diff against
    vqa_sycophancy_probe_hf_local.py, where this is actually wired in for
    'image', stays legible."""
...
def build_call_messages(messages: list[dict], evidence: str) -> list[dict]:
    """NO-EVICTION VARIANT: never strips, for either evidence mode. Every
    image ever attached stays live for the rest of the conversation. 'none'
    is unaffected by this (it never needed stripping anyway); 'image'
    becomes what we've called dual_persist elsewhere — see module
    docstring."""
    return messages
...
def image_for_pushback_turn(evidence: str, turn_index: int, real_b64: str, fake_b64: str | None) -> str | None:
    ...
    (1-indexed). 'none': never — the real turn-0 image just stays resent
    every turn since nothing evicts it. 'image': the fake proof image, turn 1
    only — but since nothing evicts it either now, it and the real turn-0
    image both stay live for every turn after too (this file's whole point —
...
def _selftest() -> None:
    ...
    # strip_images_except_last() itself is untouched/still correct — kept only
    # for the diff's sake, no longer wired into build_call_messages() below.
    stripped = strip_images_except_last(fake_history)
    assert stripped[0]["content"] == "q1"
    assert isinstance(stripped[2]["content"], list)
    # NO-EVICTION VARIANT: build_call_messages() never strips now, for either
    # evidence mode — this is the one behavior change this file makes.
    assert build_call_messages(fake_history, "image") == fake_history
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="VLM sycophancy flip probe over VQA-RAD — local in-process inference via transformers, "
                     "no-eviction variant: --evidence image / none only, any Hub vision-language model"
    )
    ...
    p.add_argument("--evidence", choices=["image", "none"], default="image",
                    help="'image': fake prescription attached turn 1, real image from turn 0 - both stay live "
                         "for the rest of the conversation since nothing evicts them here (this file's whole "
                         "point, see module docstring). 'none': pure language pushback, real image resent every turn")
