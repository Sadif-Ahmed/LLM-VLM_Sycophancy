...
The 3 local-GPU launchers take `--model MODEL` to run a single model; **omit it and they loop the 9 locally-runnable VLMs** from [`LOCAL_VLM_CONVERSATIONAL_RANKING.md`](LOCAL_VLM_CONVERSATIONAL_RANKING.md) — Qwen2.5-VL-3B/7B, Qwen3-VL-8B, medgemma-4b-it, llava-1.5-7b, HuatuoGPT-Vision-7B, MiniCPM-V-4.6, Molmo-7B-D, InternVL3-8B (excludes Llama-3.2-11B-Vision, kept on NIM, and NVLM-D-72B, too big for local; Phi-3.5-vision dropped — its Hub modeling code is incompatible with transformers 5.x; MiniCPM-V-2.6 swapped for 4.6, which is natively integrated — the 2.6 remote code needs a `.chat()` API the generic probe can't drive). Edit `MODEL_LIST` inside a `.bat` to change it. `--load-in-4bit` / `--trust-remote-code` are global — pass both when sweeping the whole list (most 7-8B models need 4-bit on an 8GB card; Molmo / InternVL3 need trust-remote-code).
...
Multi-image support (needed for `dual`, and for the faithful non-evicting `image`): Qwen2.5-VL / Qwen3-VL, MedGemma (Gemma3 backbone), InternVL3-8B, MiniCPM-V-4.6, and `nvidia/nemotron-3-nano-omni` all support it (Phi-3.5-Vision also does, but is dropped from the sweep — transformers 5.x incompatible). **Llama-3.2-Vision (11B / 90B) does not** (cross-attention architecture) — it is limited to the evicting conditions; the cross-model comparison on those stays fully populated for every model.
