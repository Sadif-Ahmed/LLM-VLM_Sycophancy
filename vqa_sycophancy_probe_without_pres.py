...
def main() -> None:
    ...
    # variant="no_pres" keeps this ablation's results fully separate from
    # vqa_sycophancy_probe.py's image-evidence runs, even though both read
    # the same underlying --dataset-dir, so RESULTS.txt in each leaf never
    # mixes the two conditions together.
    leaf_dir, transcripts_dir = output_paths(args.model, "no_pres", args.prompt_set)
    try:
