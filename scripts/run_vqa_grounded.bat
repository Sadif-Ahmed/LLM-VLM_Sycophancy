@echo off
REM ============================================================
REM run_vqa_grounded.bat
REM
REM Windows port of run_vqa_grounded.sh. Runs
REM vqa_sycophancy_probe_grounded.py across all 3 prompt-set variants:
...
REM run_vqa_probe.bat / run_vqa_without_pres.bat, instead of waiting
REM on them serially.
REM
REM Usage, from anywhere:
REM     scripts\run_vqa_grounded.bat --model MODEL [options]
REM
...
set "SCRIPT_PY=%REPO_ROOT%\vqa_sycophancy_probe_grounded.py"
...
if not exist "%SCRIPT_PY%" (
    echo [error] vqa_sycophancy_probe_grounded.py not found at "%SCRIPT_PY%"
    exit /b 1
)
