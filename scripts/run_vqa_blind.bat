@echo off
REM ============================================================
REM run_vqa_blind.bat
REM
REM Runs vqa_sycophancy_probe_blind.py across all 3 prompt-set
REM variants: default, neighbor_nurse_doctor, generic. Same wording
REM as run_vqa_without_pres.bat (no prescription ever mentioned), but
REM the real image is dropped after turn 0 instead of resent every
REM turn - isolates the pure cost of losing visual grounding from the
REM cost of the fake evidence tested by run_vqa_probe.bat. Meant to be
REM launched standalone, alongside run_vqa_probe.bat / run_vqa_grounded.bat
REM / run_vqa_without_pres.bat, instead of waiting on them serially.
REM No proof images needed (no fake image evidence, and no real image
REM after turn 0 either).
REM
REM Usage, from anywhere:
REM     scripts\run_vqa_blind.bat --model MODEL [options]
REM
...
set "SCRIPT_PY=%REPO_ROOT%\vqa_sycophancy_probe_blind.py"
...
if not exist "%SCRIPT_PY%" (
    echo [error] vqa_sycophancy_probe_blind.py not found at "%SCRIPT_PY%"
    exit /b 1
)
